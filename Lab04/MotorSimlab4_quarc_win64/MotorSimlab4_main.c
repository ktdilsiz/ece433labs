...
typedef RT_MODEL_MotorSimlab4_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * MotorSimlab4(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(MotorSimlab4_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(MotorSimlab4_DW.HILReadAnalogTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://MotorSimlab4:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: MotorSimlab4 -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(MotorSimlab4_DW.HILReadAnalogTimebase_Task,
        (t_clock) MotorSimlab4_P.HILReadAnalogTimebase_clock, 1000.0, -1)
        ;
      if (result == 0) {
