...
typedef RT_MODEL_lab7_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * lab7(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(lab7_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(lab7_DW.HILReadAnalogTimebase1_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://lab7:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: lab7 -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(lab7_DW.HILReadAnalogTimebase1_Task, (t_clock)
        lab7_P.HILReadAnalogTimebase1_clock, 500.0, -1)
        ;
      if (result == 0) {
