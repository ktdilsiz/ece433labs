...
typedef RT_MODEL_Lab_Exam_2_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * Lab_Exam_2(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(Lab_Exam_2_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(Lab_Exam_2_DW.HILReadAnalogTimebase1_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://Lab_Exam_2:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: Lab_Exam_2 -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(Lab_Exam_2_DW.HILReadAnalogTimebase1_Task,
        (t_clock) Lab_Exam_2_P.HILReadAnalogTimebase1_clock, 1000.0, -1)
        ;
      if (result == 0) {
