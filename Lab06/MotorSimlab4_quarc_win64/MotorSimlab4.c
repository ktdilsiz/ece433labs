...
void MotorSimlab4_output(void)
{
  ...
  if (rtmIsMajorTimeStep(MotorSimlab4_M)) {
    ...
    rtb_TmpSignalConversionAtThetaF[0] = MotorSimlab4_B.Bias;
    rtb_TmpSignalConversionAtThetaF[1] = MotorSimlab4_B.Tacometer;
    rtb_TmpSignalConversionAtThetaF[2] = MotorSimlab4_B.Encoder;

    /* ToFile: '<Root>/ThetaFile1' */
    {
      if (!(++MotorSimlab4_DW.ThetaFile1_IWORK.Decimation % 1) &&
          (MotorSimlab4_DW.ThetaFile1_IWORK.Count*4)+1 < 100000000 ) {
        FILE *fp = (FILE *) MotorSimlab4_DW.ThetaFile1_PWORK.FilePtr;
        if (fp != (NULL)) {
          real_T u[4];
          MotorSimlab4_DW.ThetaFile1_IWORK.Decimation = 0;
          u[0] = MotorSimlab4_M->Timing.t[1];
          ...
          if (fwrite(u, sizeof(real_T), 4, fp) != 4) {
            rtmSetErrorStatus(MotorSimlab4_M,
                              "Error writing to MAT-file DataSim1.mat");
            return;
          }

          if (((++MotorSimlab4_DW.ThetaFile1_IWORK.Count)*4)+1 >= 100000000) {
            (void)fprintf(stdout,
                          "*** The ToFile block will stop logging data before\n"
                          "    the simulation has ended, because it has reached\n"
                          "    the maximum number of elements (100000000)\n"
                          "    allowed in MAT-file DataSim1.mat.\n");
          }
        }
      }
    }
...
void MotorSimlab4_initialize(void)
{
  ...
  {
    FILE *fp = (NULL);
    char fileName[509] = "DataSim1.mat";
    if ((fp = fopen(fileName, "wb")) == (NULL)) {
      rtmSetErrorStatus(MotorSimlab4_M, "Error creating .mat file DataSim1.mat");
      return;
    }

    if (rt_WriteMat4FileHeader(fp,4,0,"data")) {
      rtmSetErrorStatus(MotorSimlab4_M,
                        "Error writing mat file header to file DataSim1.mat");
      return;
    }

    MotorSimlab4_DW.ThetaFile1_IWORK.Count = 0;
    MotorSimlab4_DW.ThetaFile1_IWORK.Decimation = -1;
    MotorSimlab4_DW.ThetaFile1_PWORK.FilePtr = fp;
  }
...
void MotorSimlab4_terminate(void)
{
  ...
  /* Terminate for ToFile: '<Root>/ThetaFile1' */
  {
    ...
    if (fp != (NULL)) {
      char fileName[509] = "DataSim1.mat";
      if (fclose(fp) == EOF) {
        rtmSetErrorStatus(MotorSimlab4_M, "Error closing MAT-file DataSim1.mat");
        return;
      }

      if ((fp = fopen(fileName, "r+b")) == (NULL)) {
        rtmSetErrorStatus(MotorSimlab4_M,
                          "Error reopening MAT-file DataSim1.mat");
        return;
      }

      if (rt_WriteMat4FileHeader(fp, 4, MotorSimlab4_DW.ThetaFile1_IWORK.Count,
           "data")) {
        rtmSetErrorStatus(MotorSimlab4_M,
                          "Error writing header for data to MAT-file DataSim1.mat");
      }
      ...
      MotorSimlab4_DW.ThetaFile1_PWORK.FilePtr = (NULL);
    }
  }
...
RT_MODEL_MotorSimlab4_T *MotorSimlab4(void)
{
  ...
  /* External mode info */
  MotorSimlab4_M->Sizes.checksums[0] = (1215567755U);
  MotorSimlab4_M->Sizes.checksums[1] = (1326878852U);
  MotorSimlab4_M->Sizes.checksums[2] = (196123178U);
  MotorSimlab4_M->Sizes.checksums[3] = (2803800737U);
  ...
  MotorSimlab4_M->Sizes.numBlocks = (30);/* Number of blocks */
